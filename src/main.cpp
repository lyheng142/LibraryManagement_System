...
static uint32_t visW(const string& s) {
    ...
    while (i < s.size()) {
        ...
        if (c == 0x1B && i+1 < s.size() && s[i+1]=='[') {
            i+=2; while (i<s.size()&&s[i]!='m') i++; i++; continue;
        }
...
static string icon(const string& l){
    ...
    if(lo.find("student") !=string::npos) return u8"🎓";
    return u8"⭐";
}
...
static void printDashMenu(const string& title,
                          ...
                          Color borderCol) {
    const char* BC=ac(borderCol); const char* RST=SP_RESET;
    ...
    auto bl=[&](const string& s){cout<<pad<<BC<<SP_BOLD<<s<<RST<<"\n";};
    ...
    bl(ts); cr("No"," Options",SP_YELLOW,true); bl(hs);
...
static void printLibrarianStats() {
    ...
    for (const auto& br : borrows)
        if (br.isActive()) active++;   // OOP method

    printCard("LIBRARY STATUS", {
        {"Total Titles",   to_string(titles)},
        {"Total Copies",   to_string(copies)},
        {"Out of Stock",   to_string(outStock)},
        {"Active Borrows", to_string(active)}
    }, Color::magenta);
}
...
static void printStudentStats(const string& username) {
    ...
    for (const auto& br : borrows) {
        ...
        if (br.belongsTo(username)) {
            myTotal++;
            if (br.isActive()) myActive++;
        }
    }
    printCard("MY ACCOUNT", {
        {"Username",    username},
        {"My Borrows",  to_string(myTotal)},
        {"Active",      to_string(myActive)}
    }, Color::cyan);
}
...
static void doRestore(vector<Book>& books, vector<Borrow>& borrows) {
    ...
    t[0].format().font_style({FontStyle::bold})
        .font_color(Color::yellow).border_bottom_color(Color::yellow);
...
void librarianMenu() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 9:  doBackup(); break;
            case 10: doRestore(books, borrows); break;
            case 11:
                clearScreen();
            ...
            default: printStatus("Invalid choice! Choose 1-11.", false);
        }
        if (choice >= 1 && choice <= 8) pressEnter();
...
void studentMenu(const string& username) {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 4: borrowBook(books,borrows,username);
                    saveBooks(books); saveBorrows(borrows);        break;
            case 5: returnBook(books,borrows,username);
                    saveBooks(books); saveBorrows(borrows);        break;
            ...
            default: printStatus("Invalid choice! Choose 1-7.", false);
        }
        if (choice >= 1 && choice <= 6) pressEnter();
...
static string chooseRole(const string& heading, Color bc) {
    ...
    do {
        ...
        if(ch==1) return "librarian";
        if(ch==2) return "student";
        if(ch==3) return "";
    } while(true);
}
...
static void runAccessPortal() {
    ...
    do {
        ...
        note[0].format().font_align(FontAlign::center).font_style({FontStyle::bold})
            .font_color(Color::magenta).border_color(Color::magenta)
...
static bool doLogin(const string& role, string& outUser) {
    ...
    if(ok){
        ...
        d[0].format().font_align(FontAlign::center).font_style({FontStyle::bold})
            .font_color(Color::red).border_color(Color::red)
            .padding_top(1).padding_bottom(1).width(TBL_W);
...
static void runMainMenu() {
    ...
    do {
        ...
        switch(ch){
            case 1: if(doLogin("librarian",loggedUser)) librarianMenu(); break;
            case 2: if(doLogin("student",loggedUser))   studentMenu(loggedUser); break;
            case 3:
            saveBooks(books);
                clearScreen(); printFigletTitle("GOODBYE",SP_CYAN);
